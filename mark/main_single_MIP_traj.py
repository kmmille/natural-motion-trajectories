...
# -*- coding: utf-8 -*-
"""
main script for visualizing MIP trajectories
...
from controllers.MIQP_to_NMT import Controller
...
ax1.plot(X[0,:],X[1,:],X[2,:], color='green', linewidth=line_width/1.5, alpha=0.7)
# vmag = (X[3,:]**2 + X[4,:]**2)**(0.5)
vmag = np.maximum(np.abs(X[3,:]), np.abs(X[4,:]))
